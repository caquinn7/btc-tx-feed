...
1. Start ETS with **empty counters** every time (remove `load_snapshot/0` call)
2. Record `started_at = DateTime.utc_now()` in the GenServer state
...
- `TxStats`: remove `persist_snapshot/0`, `load_snapshot/0`, `schedule_flush/0`, the `:flush`
  `handle_info` clause, and the `Process.flag(:trap_exit, true)` call (no longer needed since
  `terminate/2` now calls into Ecto rather than doing raw file I/O)
